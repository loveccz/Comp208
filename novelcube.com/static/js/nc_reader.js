...
function loadAd(adid) {
    var url = "//adsdk.dmzj.com/SDK/Show/play_count?gameid=1770001&channel=m&channelid=9556&adid="+adid+"&eventType=displaySuccess"
    $.ajax({
        type: "get",
        url: url,
        dataType: "json",
...
function playAd(ad_url, posid, adid){
    var url = "/ad/play.html";
    var winRef = window.open(ad_url, "_blank");//打开一个新的页面
    $.ajax({
        type: "get",
        url: url,
        data: {
        ...
        success: function (data) {
            ...
            if(ad_url!='0'){
                function loc(){
                    winRef.location = ad_url;//改变页面的 location
                }
