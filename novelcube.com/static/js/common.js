...
function UserCookie(){
    if($.cookie("my")!=null){
        ...
        nc_global.userId = cookie_arr[0];
        nc_global.username = URLdecode(cookie_arr[1]);
        nc_global.userPhoto = "https://avatar.dmzj.com/"+cookie_arr[3].substr(0,2)+"/"+cookie_arr[3].substr(2,2)+"/"+cookie_arr[3]+".png";
        nc_global.isLogin = true;
...
function subscribeUnread(){
    ...
    T.restGet(url, {}, function(data){
        if(data.unreadCount!=0 && nc_global.isLogin==true){
            $("#subscribe").show();
        }else{
            $("#subscribe").hide();
        }
...
var app_ad={
    ...
    app_home_ad:function(){
        ...
        if($.cookie("app_home_ad")==null){
            $("#app_home_show").show(function(){
                var url = "//adsdk.dmzj.com/mSdk/show";
                $.ajax({
                    type: "get",
                    url: url,
                    data: {
    ...
    openApp:function(){
        var url = "//m.dmzj.com/ad/play.html";
        $.ajax({
            type: "get",
            url: url,
            data: {
...
//                console.log('调用android');
                    window.location = "dmzj://dmzj.com/index";
                }
            },
            error: function(data) {
                console.log(data)
...
//                console.log('调用android');
        ...
        // window.location.href = "https://hapjs.org/app/com.zztc.company.dmzj"
        // return
